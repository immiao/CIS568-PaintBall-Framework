...
public class MobileShooter : MonoBehaviour {
    ...
    public void ShootBall(Vector3 velocity)
    {
        ...
        //   upon hitting a wall.
        var ball = PhotonNetwork.Instantiate("ball", ARCamera.transform.position, Quaternion.identity, 0);
        PhotonView pv = ball.GetComponent<PhotonView>();
        pv.RPC("RPCInitialize", PhotonTargets.All, velocity, new Vector3(1.0f, 0.0f, 0.0f));
       //ball.GetComponent<>
    }
    ...
    {
        if (ok)
            GUILayout.Label("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx" + ARCamera.transform.forward);
    }
